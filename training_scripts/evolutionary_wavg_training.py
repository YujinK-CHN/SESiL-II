...
if __name__ == "__main__":
    ...
    for g in range(num_generation):
        ...
        csv_file = os.path.join(
            ...
            'evolutionary_wavg_configurations.csv'
        )
        ...
        with torch.no_grad():
            for node_config in experiment_configs:
                ...
                offsprings = run_auxiliary_experiment(
                    merging_fn='match_tensors_identity', # change here
                    node_config=node_config, 
                    experiment_config=raw_config, 
                    pairs=pairs, 
                    loners=loners,
        ...
        for model_name, model in offsprings.items():
            ...
            save_dir = os.path.join(f'./checkpoints/cifar10_evolution/wavg/gen_{g+1+start_from}/{encode_labels(model_name)}')
            ...
            raw_config['model']['dir'] = f'./checkpoints/cifar10_evolution/wavg/gen_{g+1+start_from}/'
