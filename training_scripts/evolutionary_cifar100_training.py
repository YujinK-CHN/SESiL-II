...
def train_logits(model, train_loader, test_loader, epochs=5, remap_class_idxs=None):
...
if __name__ == "__main__":
    ...
    for g in range(num_generation):
        ...
        csv_file = os.path.join(
            ...
            f'evolutionary_{file_id}_configurations.csv'
        )
        ...
        with torch.no_grad():
            for node_config in experiment_configs:
                ...
                offsprings = run_auxiliary_experiment(
                    merging_fn=f'match_tensors_{file_id}', # change here
                    node_config=node_config, 
                    experiment_config=raw_config, 
                    pairs=pairs, 
                    loners=loners,
        ...
        for model_name, model in offsprings.items():
            ...
            save_dir = os.path.join(f'./checkpoints/cifar10_evolution/{file_id}/gen_{g+1+start_from}/{encode_labels(model_name)}')
            ...
            raw_config['model']['dir'] = f'./checkpoints/cifar10_evolution/{file_id}/gen_{g+1+start_from}/'
