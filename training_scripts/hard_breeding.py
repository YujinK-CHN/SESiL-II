...
def breeding(population_info, key="Joint", rng=None):
    """
    Breeding: each parent is chosen independently with probability 
    proportional to its performance score. No self-pairing allowed.
    
    Args:
        population_info: list of dicts, each with fields including "Model Name" and `key`.
        key: which metric to use for fitness ("Joint" or "Per Task Avg").
    ...
    scores = np.array([float(p[key]) for p in population_info], dtype=float)
    
    # Handle zero/negative scores: shift + normalize
    ...
    num_pairs = len(names)  # preserve population size
    pairs = []
    
    for _ in range(num_pairs):
        a = rng.choice(names, p=probs)
        # re-normalize excluding 'a'
        mask = np.array(names) != a
        probs_b = probs[mask]
        probs_b = probs_b / probs_b.sum()  # renormalize
        b = rng.choice(np.array(names)[mask], p=probs_b)
        pairs.append((a, b))
...
def mate_with_population_info(population_info):
    pairs = breeding(population_info)  # 10 pairs
    # Example output:
...
if __name__ == "__main__":
    ...
    for g in range(num_generation):
        ...
        for model_name, model in offsprings.items():
            ...
            save_dir = os.path.join(f'./checkpoints/cifar10_evolution/hard_breed/gen_{g+1+start_from}/{encode_labels(model_name)}')
            ...
            raw_config['model']['dir'] = f'./checkpoints/cifar10_evolution/hard_breed/gen_{g+1+start_from}/'
