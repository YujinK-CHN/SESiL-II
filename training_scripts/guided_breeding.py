...
def breeding_hybrid(population_info, scores, key="Joint", rng=None):
    """
    Hybrid breeding:
    - Parent A chosen with probability proportional to its performance (from population_info).
    - Parent B chosen with probability proportional to score[A][B].
    - No self-pairing allowed.
    
    Args:
        population_info: list of dicts with fields including "Model Name" and `key`.
        scores: dict of dicts, scores[a][b] = compatibility/score of b given a.
        key: performance metric from population_info (e.g., "Joint", "Per Task Avg").
        rng: numpy random generator
    
    Returns:
        pairs: list of tuples (A, B)
    """
    if rng is None:
        rng = np.random.default_rng()
    
    # Extract names and performance scores
    names = [p["Model Name"] for p in population_info]
    perf = np.array([float(p[key]) for p in population_info], dtype=float)

    # Normalize performance into probabilities
    perf = np.maximum(perf, 0)
    if perf.sum() == 0:
        probs = np.ones_like(perf) / len(perf)
    else:
        probs = perf / perf.sum()
    ...
    for _ in range(num_pairs):
        # Pick parent A from population performance
        a = rng.choice(names, p=probs)

        # Candidate mates (exclude self)
        mates = [m for m in scores[a].keys() if m != a]
        mate_scores = np.array([scores[a][m] for m in mates], dtype=float)

        # Normalize scores
        mate_scores = np.maximum(mate_scores, 0)
        if mate_scores.sum() == 0:
            probs_b = np.ones_like(mate_scores) / len(mate_scores)
        else:
            probs_b = mate_scores / mate_scores.sum()

        # Pick parent B based on scores[a][b]
        b = rng.choice(mates, p=probs_b)
...
def mate_with_population_info(population_info):
    ...
    scores = build_score_matrix(population_info, mode="threshold", threshold=0.1)
    print("Scores:\n", scores)
    ...
    pairs = breeding_hybrid(population_info, scores, key="Joint", rng=None)  # 10 pairs
    # Example output:
...
def finetune_merged_model(model, trainloader, testloader, epochs=1):
    model.train()
...
if __name__ == "__main__":
    ...
    config_name = 'cifar100_evolution_resnet20'
    skip_pair_idxs = [0]
    model_width = 4
    num_generation = 13
    start_from = 12
    ...
    for g in range(num_generation):
        ...
        file_id = "guided_breed"
        csv_file = os.path.join(
            f'./csvs/{time.strftime("%Y-%m-%d")}/{file_id}/{g+1+start_from}',
            'evolutionary_permute_configurations.csv'
        )
        ...
        for model_name, model in offsprings.items():
            ...
            save_dir = os.path.join(f'./checkpoints/cifar10_evolution/guided_breed/gen_{g+1+start_from}/{encode_labels(model_name)}')
            ...
            raw_config['model']['dir'] = f'./checkpoints/cifar10_evolution/guided_breed/gen_{g+1+start_from}/'
