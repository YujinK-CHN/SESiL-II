...
def breeding(scores, rng=None):
    """
    Probabilistic free breeding: each model mates based on scores.
    Population size preserved by making num_pairs = len(models).
    ...
    Returns:
        pairs: list of tuples (A, B) where A and B are model names
    """
    if rng is None:
        rng = np.random.default_rng()
        
    models = list(scores.keys())
    num_pairs = len(models)   # preserve population size
    ...
    for _ in range(num_pairs):
        # Pick a "parent" model
        a = rng.choice(models)
        # Choose mate based on its scores
        mates = list(scores[a].keys())
        probs = np.array(list(scores[a].values()), dtype=float)
        
        if probs.sum() == 0:
            b = rng.choice(mates)  # fallback uniform choice
        else:
            probs /= probs.sum()
            b = rng.choice(mates, p=probs)
...
def mate_with_population_info(population_info):
    ...
    pairs = breeding(scores)  # 10 pairs
    # Example output:
...
def train_logits(model, train_loader, test_loader, epochs=5, remap_class_idxs=None, restrict_class=None):
    ...
    for epoch in pbar:
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            # ✅ filter batches to only include the chosen class
            if restrict_class is not None:
                mask = labels == restrict_class
                if not mask.any():
                    continue
...
def finetune_merged_model(model, trainloader, testloader, epochs=2):
    # ✅ randomly pick one class from the training set
    all_classes = []
    for _, labels in trainloader:
        all_classes.extend(labels.tolist())
    chosen_class = random.choice(list(set(all_classes)))
    print(f"🔀 Fine-tuning only on class {chosen_class}")
    ...
    model, final_acc = train_logits(
        ...
        epochs=epochs,
        restrict_class=chosen_class,   # ✅ new argument
    )
    return model
...
if __name__ == "__main__":
    ...
    for g in range(num_generation):
        ...
        csv_file = os.path.join(
            ...
            'evolutionary_breed_configurations.csv'
        )
        ...
        for model_name, model in offsprings.items():
            ...
            save_dir = os.path.join(f'./checkpoints/cifar10_evolution/breed/gen_{g+1+start_from}/{encode_labels(model_name)}')
            ...
            raw_config['model']['dir'] = f'./checkpoints/cifar10_evolution/breed/gen_{g+1+start_from}/'
