...
def train_logits(model, train_loader, test_loader, epochs=200, remap_class_idxs=None):
    ...
    collect_acc.append(acc)
    #save_dir = "./checkpoints/CIFAR-8_baseline"
...
# Here I need to decode the name.
def inject_pair(config, pair, ignore_bases=False):
    model_name = config['model']['name']
    config['dataset']['class_splits'] = [split_str_to_ints(decode_labels(split)) for split in pair]
    if not ignore_bases:
...
def encode_labels(labels, mapping_file="mapping.json"):
    # normalize input
...
def decode_labels(hash_ids, mapping_file="mapping.json"):
    with open(mapping_file, "r") as f:
        mapping = json.load(f)

    def format_labels(labels):
        if isinstance(labels, str):
            return labels  # already a string like "0_9_8"
        return "_".join(map(str, labels))

    def decode_one(hid):
        if "_" in str(hid) and str(hid) not in mapping:
            return str(hid)
        return format_labels(mapping[str(hid)])

    if isinstance(hash_ids, str):
        return decode_one(hash_ids)
    elif isinstance(hash_ids, (list, tuple)):
        return [decode_one(hid) for hid in hash_ids]
    else:
